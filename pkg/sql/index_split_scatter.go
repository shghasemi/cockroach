...
func (is *indexSplitAndScatter) rangeMaxBytesForTable(table catalog.TableDescriptor) int64 {
	if is.testingKnobs != nil && is.testingKnobs.DisableBackfillerSplitSizeCheck {
		return 0
	}
...
func (is *indexSplitAndScatter) MaybeSplitIndexSpans(
	...
	if estimated := is.estimatedTableSize(ctx, table); estimated < is.rangeMaxBytesForTable(table) {
		log.Dev.Infof(ctx,
			"skipping index split for tableId=%d index=%d: estimated table size %d bytes fits in one range",
			table.GetID(), indexToBackfill.GetID(), estimated)
		return nil
	}
